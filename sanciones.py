...
def embed_sancion(reg: dict, guild: Optional[discord.Guild] = None) -> discord.Embed:
    """Embed profesional y consistente para una sanción."""
    estado = "❌ ANULADA" if reg.get("anulada") else ("✅ ACTIVA" if reg.get("activa") else "⏹ INACTIVA")
    color = "error" if not reg.get("anulada") else "neutral"
    titulo = f"⚖️ Sanción #{reg.get('id')} — {reg.get('tipo', 'General')}"

    desc_parts = [
        f"**ID:** `{reg.get('id')}`",
        f"**Estado:** {estado}",
        f"**Tipo:** {reg.get('tipo', '—')}",
        f"**Motivo:** {reg.get('motivo', '—')}",
    ]
    if reg.get("duracion"):
        desc_parts.append(f"**Duración:** {reg['duracion']}")
    if reg.get("evidencia"):
        desc_parts.append(f"**Evidencia (texto):** {reg['evidencia']}")

    embed = crear_embed(color, titulo, "\n".join(desc_parts))
    ...
    embed.add_field(name="Fecha", value=str(reg.get("fecha", "—"))[:19].replace("T", " ") + " UTC", inline=True)

    urls = reg.get("evidencia_urls") or []
    if urls:
        links = "\n".join(f"[Archivo {i+1}]({u})" for i, u in enumerate(urls[:8]))
        embed.add_field(name="📎 Evidencia (archivos)", value=links, inline=False)
        # Primera imagen como thumbnail si parece imagen
        first = urls[0]
    ...
    if reg.get("anulada"):
        embed.add_field(
            name="Anulada",
            value=f"Por <@{reg.get('anulada_por')}> — {reg.get('anulada_motivo', '—')}\n{str(reg.get('anulada_fecha', ''))[:19]}",
            inline=False,
        )

    embed.set_footer(text=f"{config.NOMBRE_HOSPITAL} · Sistema de sanciones")
    return embed
