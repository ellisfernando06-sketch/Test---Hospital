# -*- coding: utf-8 -*-
"""comandos_nuevos.py — Registro de comandos (sanciones, verificacion Roblox, moderacion)."""
...
def registrar(bot: commands.Bot) -> None:
    ...
    @app_commands.describe(
        ...
        duracion="Duracion si aplica (ej: 3 dias)",
    )
    ...
    @bot.tree.command(name="quitar_sancion", description="Anula una sancion")
    @app_commands.describe(sancion_id="ID de la sancion", motivo="Motivo de la anulacion")
    ...
    async def quitar_sancion(interaction: discord.Interaction, sancion_id: int, motivo: str):
        ...
        if not reg:
            await interaction.response.send_message(f"No se encontro la sancion #{sancion_id} o ya estaba anulada.", ephemeral=True)
            return
        registros.registrar_evento_cargo(reg["usuario_id"], "sancion_anulada", f"#{sancion_id}: {motivo}", interaction.user.id)
        embed = sanciones.embed_sancion(reg, interaction.guild)
        embed.title = f"Sancion #{sancion_id} anulada"
        await interaction.response.send_message(embed=embed)
        await sanciones.enviar_log_sancion(bot, embed)

    @bot.tree.command(name="apelar_sancion", description="Apela una sancion y abre ticket")
    @app_commands.describe(sancion_id="ID de la sancion a apelar", razon="Por que debe revisarse")
    async def apelar_sancion(interaction: discord.Interaction, sancion_id: int, razon: str):
        ...
        if not reg:
            await interaction.response.send_message(f"No existe la sancion #{sancion_id}.", ephemeral=True)
            return
        if reg.get("anulada"):
            await interaction.response.send_message(f"La sancion #{sancion_id} ya fue anulada.", ephemeral=True)
            return
        if int(reg.get("usuario_id", 0)) != interaction.user.id:
            if not isinstance(interaction.user, discord.Member) or not permisos.member_tiene_alguna_key(
                ...
                await interaction.response.send_message("Solo puedes apelar tus propias sanciones (o ser staff).", ephemeral=True)
                return
        ...
        if not canal:
            await interaction.followup.send("No pude crear el ticket de apelacion.", ephemeral=True)
            return
        await canal.send(f"**Razon de la apelacion:**\n{razon}\n\nPuedes adjuntar mas evidencias aqui.")
        await interaction.followup.send(f"Ticket de apelacion creado: {canal.mention}", ephemeral=True)

    @bot.tree.command(name="banear", description="Banea a un miembro")
    @app_commands.describe(usuario="Usuario a banear", motivo="Motivo", borrar_dias="Dias de mensajes a borrar (0-7)")
    @require_key("DIRECTOR_DISCIPLINA", "DIRECTOR_ADMINISTRATIVO", "DIRECTOR_GENERAL", "OWNER", "CO_OWNER")
    async def banear(interaction: discord.Interaction, usuario: discord.Member, motivo: str, borrar_dias: int = 0):
        if not permisos.puede_actuar_sobre(interaction.user, usuario):
        ...
        embed = crear_embed("error", "Usuario baneado", f"**Usuario:** {usuario} (`{usuario.id}`)\n**Motivo:** {motivo}\n**Por:** {interaction.user.mention}", autor=interaction.user)
        await interaction.response.send_message(embed=embed)

    @bot.tree.command(name="expulsar", description="Expulsa (kick) a un miembro")
    ...
    async def expulsar(interaction: discord.Interaction, usuario: discord.Member, motivo: str):
        ...
        embed = crear_embed("error", "Usuario expulsado", f"**Usuario:** {usuario} (`{usuario.id}`)\n**Motivo:** {motivo}\n**Por:** {interaction.user.mention}", autor=interaction.user)
        await interaction.response.send_message(embed=embed)

    @bot.tree.command(name="silenciar", description="Silencia (timeout) a un miembro")
    @app_commands.describe(usuario="Usuario a silenciar", minutos="Minutos de silencio", motivo="Motivo")
    @require_key("DIRECTOR_DISCIPLINA", "DIRECTOR_ADMINISTRATIVO", "DIRECTOR_GENERAL", "SUPERVISOR", "OWNER", "CO_OWNER")
    async def silenciar(interaction: discord.Interaction, usuario: discord.Member, minutos: int, motivo: str):
        if not permisos.puede_actuar_sobre(interaction.user, usuario):
        ...
        try:
            ...
            return
        registros.registrar_evento_cargo(usuario.id, "timeout", f"{minutos} min: {motivo}", interaction.user.id)
        embed = crear_embed("aviso", "Usuario silenciado", f"**Usuario:** {usuario.mention}\n**Duracion:** {minutos} min\n**Motivo:** {motivo}\n**Por:** {interaction.user.mention}", autor=interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def quitar_silencio(interaction: discord.Interaction, usuario: discord.Member):
        try:
            ...
            return
        await interaction.response.send_message(f"Se quito el silencio a {usuario.mention}.")

    @bot.tree.command(name="verificar_roblox", description="Envia DM de verificacion Roblox a Visitantes")
    @app_commands.describe(usuario="Un visitante especifico (opcional)")
    ...
    async def verificar_roblox(interaction: discord.Interaction, usuario: Optional[discord.Member] = None):
        ...
        if not r_vis:
            await interaction.response.send_message("No encontre el rol Visitante.", ephemeral=True)
            return
        if not r_miem:
            await interaction.response.send_message("No encontre el rol Miembro.", ephemeral=True)
            return
        if usuario:
            if r_vis not in usuario.roles:
                await interaction.response.send_message(f"{usuario.mention} no tiene el rol Visitante.", ephemeral=True)
                return
            ok = await verificacion.enviar_dm_verificacion(usuario, interaction.user)
            if ok:
                await interaction.response.send_message(f"DM de verificacion enviado a {usuario.mention}.", ephemeral=True)
            else:
                await interaction.response.send_message(f"No pude enviarle DM a {usuario.mention} (DMs cerrados).", ephemeral=True)
            return
        visitantes = [m for m in guild.members if r_vis in m.roles and not m.bot]
        if not visitantes:
            await interaction.response.send_message("No hay miembros con rol Visitante.", ephemeral=True)
            return
        enviados = 0
        fallidos = 0
        await interaction.response.defer(ephemeral=True)
        ...
        await interaction.followup.send(
            f"DMs enviados: {enviados} | Fallidos: {fallidos}\nVisitante=`{r_vis.name}` -> Miembro=`{r_miem.name}`",
            ephemeral=True,
        )
    ...
    async def ver_roblox(interaction: discord.Interaction, usuario: discord.Member):
        ...
        embed = embed_roblox_verificacion(discord_user=usuario, roblox_data=roblox_data, staff=None, aprobado=True)
