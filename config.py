...
sistema de permisos por "keys" (llaves) según el cargo de cada quien:
    ...
    OWNER                              -> control total
    JEFE_JUNTA_DIRECTIVA (presidente)  -> control administrativo total
    JUNTA_DIRECTIVA (consejo)          -> acciones de junta / votaciones
...
Antes de ejecutar:
    1. Completa config.py con tu TOKEN y ajusta departamentos/keys si
       hace falta (los nombres, no los IDs).
    2. En el Developer Portal, activa "Server Members Intent".
    3. Ejecuta /configurar_roles una vez en tu servidor: crea todos
       los roles automáticamente.
    4. Sube el rol del bot por encima de todos los roles que vaya a
       asignar/quitar/ascender/descender/suspender.
"""
...
# ---------------------------------------------------------------------------
# Se guarda fuera de la configuración para que el estado de mantenimiento
# sobreviva a un reinicio del proceso. En Railway, /data puede usarse como
# volumen persistente si quieres conservarlo entre despliegues.
ESTADO_BOT_FILE = os.getenv("BOT_STATUS_FILE", "bot_status.json")
BOT_VERSION = os.getenv("BOT_VERSION", "2.0.0")
BOT_STARTED_AT = time.time()


def _cargar_estado_bot():
    estado = {
        "modo": "online",
        "motivo": "",
...
ESTADO_BOT = _cargar_estado_bot()


def _guardar_estado_bot():
    try:
        with open(ESTADO_BOT_FILE, "w", encoding="utf-8") as f:
            json.dump(ESTADO_BOT, f, ensure_ascii=False, indent=2)
    except OSError as e:
        print(f"⚠️ No se pudo guardar el estado del bot: {e}")


def _es_owner(member):
    try:
...
def _uptime_text():
    segundos = max(0, int(time.time() - BOT_STARTED_AT))
...
def construir_embed_estado_bot(guild=None):
    modo = ESTADO_BOT.get("modo", "online")
    if modo == "online":
        icono, titulo, descripcion = "🟢", "Bot ONLINE", "El bot está operativo y aceptando comandos."
    elif modo == "mantenimiento":
        icono, titulo, descripcion = "🟡", "Bot EN MANTENIMIENTO", ESTADO_BOT.get("motivo") or "Se están realizando tareas de mantenimiento."
    else:
        icono, titulo, descripcion = "🔴", "Bot APAGADO", ESTADO_BOT.get("motivo") or "El bot está temporalmente desactivado."

    embed = discord.Embed(
        title=f"{icono} Estado del Bot",
        description=descripcion,
        timestamp=discord.utils.utcnow(),
    )
    embed.add_field(name="Estado", value=titulo, inline=True)
    embed.add_field(name="Versión", value=BOT_VERSION, inline=True)
    embed.add_field(name="Latencia", value=f"{round(bot.latency * 1000)} ms" if bot.is_ready() else "Calculando...", inline=True)
    embed.add_field(name="Uptime", value=_uptime_text(), inline=True)
    embed.add_field(name="Comandos", value=str(len(bot.tree.get_commands())), inline=True)
    embed.add_field(name="Servidor", value=guild.name if guild else "Global", inline=True)
    embed.set_footer(text="Panel automático de estado")
    return embed


class HospitalCommandTree(discord.app_commands.CommandTree):
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # OWNER siempre puede administrar el bot, incluso durante mantenimiento.
        if _es_owner(interaction.user):
            return True
        ...
        if modo == "mantenimiento":
            motivo = ESTADO_BOT.get("motivo") or "Se están realizando tareas de mantenimiento."
            await interaction.response.send_message(
                f"🟡 **Bot en mantenimiento**\n{motivo}\n\nInténtalo nuevamente cuando el mantenimiento termine.",
                ephemeral=True,
            )
        else:
            motivo = ESTADO_BOT.get("motivo") or "El bot está temporalmente apagado."
            await interaction.response.send_message(
                f"🔴 **Bot apagado**\n{motivo}",
                ephemeral=True,
            )
...
bot = commands.Bot(command_prefix="!", intents=intents, tree_cls=HospitalCommandTree)
...
@bot.tree.command(name="configurar_roles", description="Crea/detecta automáticamente todos los roles del hospital")
@require_key("OWNER")
...
@require_key(
    "OWNER", "JEFE_JUNTA_DIRECTIVA", "JUNTA_DIRECTIVA",
...
async def otorgar_key(interaction: discord.Interaction, usuario: discord.Member, key: app_commands.Choice[str]):
    ...
    if not role_id or not interaction.guild.get_role(role_id):
        await interaction.response.send_message(
            "❌ Esa key todavía no tiene un rol creado. Ejecuta `/configurar_roles` primero.",
            ephemeral=True,
        )
        return
...
# (Junta Directiva / Administración), usada por cartas de solicitud.
SOLICITUD_CHOICES = DEPARTAMENTO_CHOICES + [
    app_commands.Choice(name="General / Junta Directiva", value="general")
]


async def _puede_gestionar_departamento(interaction: discord.Interaction, slug: str) -> bool:
    emisor = interaction.user
    if permisos.member_tiene_alguna_key(emisor, "OWNER", "JEFE_JUNTA_DIRECTIVA"):
        return True
...
async def ascenso(interaction: discord.Interaction, usuario: discord.Member, departamento: app_commands.Choice[str]):
    ...
    if not await _puede_gestionar_departamento(interaction, slug):
        raise SinPermiso([config.DEPARTAMENTOS[slug]["director_key"], "JEFE_JUNTA_DIRECTIVA", "OWNER"])
...
async def transferir_departamento(
    ...
    autorizado = permisos.member_tiene_alguna_key(interaction.user, "JEFE_JUNTA_DIRECTIVA", "OWNER") or (
    ...
    if not autorizado:
        raise SinPermiso(["Director de ambos departamentos", "JEFE_JUNTA_DIRECTIVA", "OWNER"])
...
async def contratar(interaction: discord.Interaction, usuario: discord.Member, departamento: app_commands.Choice[str]):
    ...
    if not permisos.member_tiene_alguna_key(
        interaction.user, "DIRECTOR_RRHH", director_key, "JEFE_JUNTA_DIRECTIVA", "OWNER"
    ):
        raise SinPermiso(["DIRECTOR_RRHH", director_key, "JEFE_JUNTA_DIRECTIVA", "OWNER"])
...
@require_key("DIRECTOR_RRHH", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@bot.tree.command(name="suspender", description="Suspende temporalmente a un miembro del personal")
@app_commands.describe(usuario="Usuario", motivo="Motivo de la suspensión")
@require_key("SUPERVISOR", "JEFE_DEPARTAMENTO", "DIRECTOR", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@require_key(
    ...
    "JUNTA_DIRECTIVA", "JEFE_JUNTA_DIRECTIVA", "OWNER",
)
...
@require_key("DIRECTOR_MEDICO", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@require_key("DIRECTOR", "JEFE_DEPARTAMENTO", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
async def balance(interaction: discord.Interaction, usuario: discord.Member = None):
    ...
    if usuario and usuario != interaction.user:
        if not permisos.member_tiene_alguna_key(
            interaction.user, "OWNER", "JEFE_JUNTA_DIRECTIVA", "DIRECTOR_FINANCIERO", "JUNTA_DIRECTIVA"
        ):
            await interaction.response.send_message(
...
@require_key("OWNER", "JEFE_JUNTA_DIRECTIVA", "DIRECTOR_FINANCIERO")
...
@require_key("DIRECTOR_FINANCIERO", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@require_key("OWNER", "JEFE_JUNTA_DIRECTIVA", "DIRECTOR")
...
# ===========================================================================
# 7) ACCIONES DE JEFATURA (Junta Directiva)
# ===========================================================================

@bot.tree.command(name="convocar_junta", description="Convoca una reunión de la Junta Directiva")
@app_commands.describe(titulo="Motivo de la reunión", fecha_hora="Fecha y hora", detalles="Detalles adicionales (opcional)")
@require_key("JEFE_JUNTA_DIRECTIVA", "OWNER")
async def convocar_junta(interaction: discord.Interaction, titulo: str, fecha_hora: str, detalles: str = ""):
    embed = crear_embed("info", f"🏛️ Convocatoria: {titulo}", detalles, autor=interaction.user)
    embed.add_field(name="Fecha y hora", value=fecha_hora)

    junta_id = roles_store.obtener_id_key("JUNTA_DIRECTIVA")
    rol_junta = interaction.guild.get_role(junta_id) if junta_id else None
    mencion = rol_junta.mention if rol_junta else ""
...
@require_key("JEFE_JUNTA_DIRECTIVA", "DIRECTOR", "OWNER")
...
class VotacionView(discord.ui.View):
    """
    Vista de votación para la Junta Directiva. No es persistente entre
    ...
    def _autorizado(self, member: discord.Member) -> bool:
        return permisos.member_tiene_alguna_key(member, self.key_autorizada, "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@bot.tree.command(name="votacion", description="Abre una votación para la Junta Directiva")
@app_commands.describe(titulo="Tema a votar")
@require_key("JEFE_JUNTA_DIRECTIVA", "OWNER")
async def votacion(interaction: discord.Interaction, titulo: str):
    embed = crear_embed(
        "info", f"🗳️ Votación: {titulo}", "Miembros de la Junta Directiva, emitan su voto.", autor=interaction.user
    )
    embed.add_field(name="Resultados", value="✅ Sí: 0 | ❌ No: 0 | ⚪ Abstención: 0")
    await interaction.response.send_message(embed=embed, view=VotacionView("JUNTA_DIRECTIVA"))
...
@require_key("SUPERVISOR", "JEFE_DEPARTAMENTO", "DIRECTOR", "JUNTA_DIRECTIVA", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
async def solicitud_permiso(interaction: discord.Interaction):
    ...
    if slug:
        ...
        slug = "general"
        nombre = "General / Junta Directiva"
...
def _es_personal_medico(member: discord.Member) -> bool:
    if permisos.member_tiene_alguna_key(
        member, "DIRECTOR_MEDICO", "DIRECTOR_ENFERMERIA", "JEFE_JUNTA_DIRECTIVA", "OWNER"
    ):
        return True
...
@require_key("DIRECTOR_LOGISTICA", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@require_key("STAFF", "SUPERVISOR", "JEFE_DEPARTAMENTO", "DIRECTOR", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
@require_key("DIRECTOR_RRHH", "DIRECTOR", "JEFE_JUNTA_DIRECTIVA", "OWNER")
...
async def postulacion_pendientes(interaction: discord.Interaction):
    ...
    lineas = []
    for p in lista[:20]:
        nombre_depto = config.DEPARTAMENTOS.get(p["departamento_slug"], {}).get("nombre", "General / Junta Directiva")
...
@require_key("DIRECTOR", "JEFE_JUNTA_DIRECTIVA", "OWNER")
