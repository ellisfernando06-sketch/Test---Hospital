...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    # Desenvuelve errores anidados (CheckFailure, CommandInvokeError, etc.)
    original = getattr(error, "original", None) or getattr(error, "__cause__", None) or error
    if isinstance(error, SinPermiso) or isinstance(original, SinPermiso):
        sp = error if isinstance(error, SinPermiso) else original
        legibles = ", ".join(getattr(sp, "keys_requeridas", []))
        msg = f"❌ No tienes el cargo/key necesario. Se requiere: **{legibles or 'permiso superior'}**."
    elif isinstance(error, app_commands.MissingPermissions):
        msg = "❌ No tienes permiso de Discord para usar este comando."
    elif isinstance(error, app_commands.CommandOnCooldown):
        msg = f"⏳ Espera {error.retry_after:.1f}s antes de usar este comando otra vez."
    else:
        msg = f"❌ Error: `{original}`"
        print(f"[CMD ERROR] {interaction.command}: {type(error).__name__}: {error} | original={original}")
    ...
    try:
        ...
        print(f"[CMD ERROR] no se pudo responder a la interacción: {e}")
...
# Destinatarios: todas las direcciones (para solicitudes)
DESTINATARIO_CHOICES = [
    app_commands.Choice(name=label, value=key)
    for label, key in DESTINATARIOS_CHOICES
]
...
async def otorgar_key(interaction: discord.Interaction, usuario: discord.Member, key: app_commands.Choice[str]):
    ...
    roles_extra = [rol]
    # Si es un cargo de Staff del Servidor, también se otorga el rol de categoría
    if key.value in getattr(config, "KEYS_CON_STAFF_SERVIDOR", set()):
        sid = roles_store.obtener_id_key("STAFF_SERVIDOR")
        if sid:
            srol = interaction.guild.get_role(sid)
            if srol and srol not in usuario.roles:
                roles_extra.append(srol)

    try:
        await usuario.add_roles(*roles_extra, reason=f"Key otorgada por {emisor}")
    except discord.Forbidden:
        await interaction.response.send_message("❌ No tengo permisos suficientes.", ephemeral=True)
        return

    extras_txt = ""
    if len(roles_extra) > 1:
        extras_txt = " (+ " + ", ".join(r.mention for r in roles_extra[1:]) + ")"
    await interaction.response.send_message(
        f"✅ Se le otorgó la key **{key.value}** ({rol.mention}){extras_txt} a {usuario.mention}."
    )
    await enviar_log("log_roles", crear_embed(
        "exito", "Key otorgada", f"**{emisor}** otorgó **{key.value}** a **{usuario}**{extras_txt}"))
...
@bot.tree.command(name="despedir", description="Solicita el despido de un usuario (requiere aprobación de RRHH)")
@app_commands.describe(usuario="Usuario a despedir", motivo="Motivo del despido", destinatario="A quién se envía la solicitud", evidencia="Evidencia o detalle adicional")
@app_commands.choices(destinatario=DESTINATARIO_CHOICES)
@require_key("DIRECTOR", "DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "DIRECTOR_ADMINISTRATIVO", "OWNER")
async def despedir(interaction: discord.Interaction, usuario: discord.Member, motivo: str, destinatario: app_commands.Choice[str], evidencia: str = ""):
    emisor = interaction.user
    if not permisos.puede_actuar_sobre(emisor, usuario):
        await interaction.response.send_message(
            "❌ No puedes solicitar el despido de alguien de tu mismo nivel o superior.", ephemeral=True
        )
        return

    # OWNER puede ejecutar directo
    if permisos.member_tiene_key(emisor, "OWNER") or permisos.member_tiene_key(emisor, "DIRECTOR_RRHH"):
        ...
        await interaction.response.send_message(f"✅ {usuario.mention} fue despedido (ejecutado por {emisor.mention}).")
        await enviar_log("log_personal", crear_embed(
            "error", "🚫 Despido", f"**{emisor}** despidió a **{usuario}**\nMotivo: {motivo}"))
        return

    embed = crear_embed(
        "aviso", "🚫 Solicitud de despido",
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            interaction,
            key_aprobador=destinatario.value,
            embed=embed,
            tipo="despido",
            datos={"usuario_id": usuario.id, "motivo": motivo, "evidencia": evidencia},
            canal_key=canal_para_key(destinatario.value),
            on_approve=_on_approve,
        )
        await interaction.followup.send(
            f"✅ Solicitud de despido de {usuario.mention} enviada a **{nombre_destinatario(destinatario.value)}**.",
            ephemeral=True,
        )
    except Exception as e:
...
@bot.tree.command(name="sancion_interna", description="Solicita sanción — elige dirección destino")
@app_commands.describe(
    usuario="Usuario sancionado",
    direccion="Dirección a la que se envía (canal de ese director)",
    motivo="Motivo de la sanción",
    evidencia="Evidencia o detalle adicional",
)
@app_commands.choices(direccion=DESTINATARIO_CHOICES)
@require_key("DIRECTOR", "DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "DIRECTOR_ADMINISTRATIVO", "SUPERVISOR", "OWNER")
async def sancion_interna(
    interaction: discord.Interaction,
    usuario: discord.Member,
    ...
    """Envía solicitud de sanción al canal del director elegido."""
    key_dest = direccion.value
    canal = canal_para_key(key_dest) or "aprobaciones_rrhh"
    titulo = f"⚠️ Sanción — {nombre_destinatario(key_dest)}"

    embed = crear_embed(
        "aviso", titulo,
        f"**Usuario:** {usuario.mention}\n**Dirección:** {nombre_destinatario(key_dest)}\n**Motivo:** {motivo}\n**Evidencia:** {evidencia or '—'}",
        autor=interaction.user,
    )
    embed.add_field(name="Destinatario", value=f"**{nombre_destinatario(key_dest)}**", inline=True)
    embed.add_field(name="Estado", value="⏳ Pendiente de aprobación", inline=True)

    async def _on_approve(inter, info):
        uid = int(info["datos"].get("usuario_id", 0))
        registros.registrar_evento_cargo(
            uid, "sancion",
            f"{info['datos'].get('motivo','')}",
            inter.user.id,
        )
        await inter.followup.send("✅ Sanción registrada en el expediente.", ephemeral=True)
        await enviar_log("log_sanciones", crear_embed(
            "aviso", f"{titulo} — aprobada",
            f"**{inter.user}** aprobó sanción a <@{uid}>\nMotivo: {info['datos'].get('motivo')}\nAprobado por: {nombre_destinatario(key_dest)}"))
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            ...
            embed=embed,
            tipo="sancion",
            datos={"usuario_id": usuario.id, "motivo": motivo, "evidencia": evidencia, "destinatario": key_dest},
            canal_key=canal,
            on_approve=_on_approve,
        )
        await interaction.followup.send(
            f"✅ Solicitud enviada al canal de **{nombre_destinatario(key_dest)}**.",
            ephemeral=True,
        )
    except Exception as e:
        await interaction.followup.send(f"❌ Error al enviar la solicitud: `{e}`", ephemeral=True)



@bot.tree.command(name="solicitud_investigacion", description="Solicita investigación — elige dirección destino")
@app_commands.describe(
    direccion="Dirección a la que se envía (canal de ese director)",
    motivo="Motivo de la investigación",
    usuario="Persona a investigar (opcional)",
    evidencia="Evidencia",
)
@app_commands.choices(direccion=DESTINATARIO_CHOICES)
@require_key("DIRECTOR", "JEFE_DEPARTAMENTO", "SUPERVISOR", "DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "DIRECTOR_ADMINISTRATIVO", "OWNER")
async def solicitud_investigacion(
    interaction: discord.Interaction,
    direccion: app_commands.Choice[str],
    motivo: str,
    ...
    key_dest = direccion.value
    canal = canal_para_key(key_dest) or "aprobaciones_rrhh"
    titulo = f"🔎 Investigación — {nombre_destinatario(key_dest)}"

    texto = f"**Motivo:** {motivo}\n**Evidencia:** {evidencia or '—'}"
    if usuario:
        texto = f"**Implicado:** {usuario.mention}\n" + texto
    embed = crear_embed("aviso", titulo, texto, autor=interaction.user)
    embed.add_field(name="Destinatario", value=f"**{nombre_destinatario(key_dest)}**", inline=True)
    embed.add_field(name="Estado", value="⏳ Pendiente de aprobación", inline=True)
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            ...
            embed=embed,
            tipo="investigacion",
            datos={"usuario_id": usuario.id if usuario else 0, "motivo": motivo, "evidencia": evidencia, "destinatario": key_dest},
            canal_key=canal,
        )
        await interaction.followup.send(
            f"✅ Investigación enviada al canal de **{nombre_destinatario(key_dest)}**.",
            ephemeral=True,
        )
    except Exception as e:
...
@bot.tree.command(name="suspender", description="Solicita suspensión temporal (aprobación RRHH)")
@app_commands.describe(usuario="Usuario", motivo="Motivo", destinatario="A quién se envía la solicitud", duracion="Duración (ej: 3 días)")
@app_commands.choices(destinatario=DESTINATARIO_CHOICES)
@require_key("DIRECTOR", "DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "OWNER")
async def suspender(interaction: discord.Interaction, usuario: discord.Member, motivo: str, destinatario: app_commands.Choice[str], duracion: str = "indefinida"):
    ...
    # RRHH / OWNER ejecutan directo
    if permisos.member_tiene_key(interaction.user, "DIRECTOR_RRHH") or permisos.member_tiene_key(interaction.user, "OWNER"):
        rol_id = roles_store.obtener_extra("SUSPENDIDO")
        rol = interaction.guild.get_role(rol_id) if rol_id else None
        if not rol:
            await interaction.response.send_message(
                "❌ Rol Suspendido no configurado. Ejecuta `/configurar_roles`.", ephemeral=True)
            return
        try:
            ...
            await interaction.response.send_message("❌ No tengo permisos suficientes.", ephemeral=True)
            return
        registros.registrar_evento_cargo(usuario.id, "suspension", f"{motivo} ({duracion})", interaction.user.id)
    ...
    embed = crear_embed(
        "aviso", "⛔ Solicitud de suspensión",
        f"**Usuario:** {usuario.mention}\n**Duración:** {duracion}\n**Motivo:** {motivo}",
        autor=interaction.user,
    )
    embed.add_field(name="Estado", value="⏳ Pendiente de aprobación de **Director de RRHH**", inline=False)
    ...
    async def _on_approve(inter, info):
        ...
        miembro = guild.get_member(uid) if guild else None
        if not miembro:
            await inter.followup.send("⚠️ Usuario no encontrado.", ephemeral=True)
            return
        rol_id = roles_store.obtener_extra("SUSPENDIDO")
        rol = guild.get_role(rol_id) if rol_id else None
        if rol:
            try:
                await miembro.add_roles(rol, reason=f"Suspensión aprobada por {inter.user}")
            except discord.Forbidden:
                await inter.followup.send("❌ Sin permisos para asignar Suspendido.", ephemeral=True)
                return
        registros.registrar_evento_cargo(miembro.id, "suspension", info["datos"].get("motivo", ""), inter.user.id)
        await inter.followup.send(f"✅ {miembro.mention} suspendido.", ephemeral=True)
        await enviar_log("log_personal", crear_embed(
            "aviso", "⛔ Suspensión aprobada",
            f"**{inter.user}** aprobó suspensión de **{miembro}**\n{info['datos'].get('motivo','')}"))
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            ...
            embed=embed,
            tipo="suspension",
            datos={"usuario_id": usuario.id, "motivo": motivo, "duracion": duracion},
            canal_key=canal_para_key(destinatario.value),
            on_approve=_on_approve,
        )
        await interaction.followup.send(
            f"✅ Solicitud de suspensión de {usuario.mention} enviada a **{nombre_destinatario(destinatario.value)}**.",
            ephemeral=True,
        )
    except Exception as e:
        await interaction.followup.send(f"❌ Error: `{e}`", ephemeral=True)
...
@require_key("DIRECTOR", "OWNER", "DIRECTOR_GENERAL", "CO_OWNER")
async def anuncio(interaction: discord.Interaction, titulo: str, mensaje: str):
    ...
    try:
        ...
        # Intenta publicar en canal de anuncios/log si está configurado; si no, aquí mismo
        canal = None
        for key in ("anuncios", "log_general", "bot_status"):
            cid = config.CANALES.get(key)
            if cid:
                canal = interaction.guild.get_channel(cid) if interaction.guild else None
                if canal:
                    break
        if canal and canal.id != interaction.channel_id:
            await canal.send(embed=embed)
        ...
        try:
            await interaction.followup.send(f"❌ No se pudo publicar el anuncio: `{e}`", ephemeral=True)
        except Exception:
            pass
...
# ===========================================================================
# STAFF DEL SERVIDOR — Citatorios, reportes y sanciones OOC
# ===========================================================================

@bot.tree.command(name="citatorio_general", description="Emite citatorio de Director General (requiere autorización)")
@require_key("DIRECTOR_GENERAL", "OWNER", "CO_OWNER")
async def citatorio_general(interaction: discord.Interaction):
    modal = CitatorioModal(
        direccion="Director General",
        key_aprobador="DIRECTOR_GENERAL",
        canal_key="citatorio_general",
        titulo="Citatorio — Director General",
    )
    await interaction.response.send_modal(modal)


@bot.tree.command(name="citatorio_disciplina", description="Emite citatorio de Director de Disciplina (requiere autorización)")
@require_key("DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "OWNER", "CO_OWNER")
async def citatorio_disciplina(interaction: discord.Interaction):
    modal = CitatorioModal(
        direccion="Director de Disciplina",
        key_aprobador="DIRECTOR_DISCIPLINA",
        canal_key="citatorio_disciplina",
        titulo="Citatorio — Disciplina",
    )
    await interaction.response.send_modal(modal)


@bot.tree.command(name="citatorio_admin", description="Emite citatorio de Director Administrativo (requiere autorización)")
@require_key("DIRECTOR_ADMINISTRATIVO", "DIRECTOR_GENERAL", "OWNER", "CO_OWNER")
async def citatorio_admin(interaction: discord.Interaction):
    modal = CitatorioModal(
        direccion="Director Administrativo",
        key_aprobador="DIRECTOR_ADMINISTRATIVO",
        canal_key="citatorio_admin",
        titulo="Citatorio — Administrativo",
    )
    await interaction.response.send_modal(modal)


@bot.tree.command(name="reporte_procedimiento", description="Publica el embed completo de un procedimiento (motivo, sanción, encargado)")
@app_commands.describe(direccion="Dirección que procedió")
@app_commands.choices(direccion=[
    app_commands.Choice(name="Director General", value="Director General"),
    app_commands.Choice(name="Director de Disciplina", value="Director de Disciplina"),
    app_commands.Choice(name="Director Administrativo", value="Director Administrativo"),
])
@require_key("DIRECTOR_GENERAL", "DIRECTOR_DISCIPLINA", "DIRECTOR_ADMINISTRATIVO", "OWNER", "CO_OWNER")
async def reporte_procedimiento(interaction: discord.Interaction, direccion: app_commands.Choice[str]):
    modal = ReporteProcedimientoModal(direccion=direccion.value)
    await interaction.response.send_modal(modal)


# --- Sanciones fuera de RP (normativa Discord) — solo Staff del Servidor / Admin ---

@bot.tree.command(name="ooc_advertencia", description="[Fuera de RP] Advertencia por infracción a la normativa del Discord")
@app_commands.describe(usuario="Usuario", motivo="Motivo de la infracción", norma="Norma infringida")
@require_key("DIRECTOR_GENERAL", "DIRECTOR_DISCIPLINA", "DIRECTOR_ADMINISTRATIVO", "OWNER", "CO_OWNER")
async def ooc_advertencia(interaction: discord.Interaction, usuario: discord.Member, motivo: str, norma: str = ""):
    registros.registrar_advertencia(usuario.id, f"[OOC] {norma}: {motivo}" if norma else f"[OOC] {motivo}", interaction.user.id)
    embed = crear_embed(
        "aviso", "⚠️ Advertencia fuera de RP (Discord)",
        f"**Usuario:** {usuario.mention}\n**Norma:** {norma or '—'}\n**Motivo:** {motivo}",
        autor=interaction.user,
    )
    await interaction.response.send_message(embed=embed)
    await enviar_log("log_sanciones_ooc", embed)
    try:
        await usuario.send(embed=embed)
    except discord.Forbidden:
        pass
...
@bot.tree.command(name="ooc_timeout", description="[Fuera de RP] Timeout (silenciar temporal) por infracción Discord")
@app_commands.describe(usuario="Usuario", minutos="Minutos de timeout", motivo="Motivo")
@require_key("DIRECTOR_GENERAL", "DIRECTOR_DISCIPLINA", "DIRECTOR_ADMINISTRATIVO", "OWNER", "CO_OWNER")
async def ooc_timeout(interaction: discord.Interaction, usuario: discord.Member, minutos: int, motivo: str):
    if minutos < 1 or minutos > 40320:
        await interaction.response.send_message("❌ Minutos entre 1 y 40320 (28 días).", ephemeral=True)
        return
    from datetime import timedelta
    try:
        await usuario.timeout(timedelta(minutes=minutos), reason=f"[OOC] {motivo} — por {interaction.user}")
    except discord.Forbidden:
        await interaction.response.send_message("❌ No tengo permiso de Timeout o el usuario es superior.", ephemeral=True)
        return
    registros.registrar_evento_cargo(usuario.id, "ooc_timeout", f"{minutos} min: {motivo}", interaction.user.id)
    embed = crear_embed(
        "error", "🔇 Timeout fuera de RP",
...
@bot.tree.command(name="ooc_kick", description="[Fuera de RP] Expulsar del servidor por infracción grave a la normativa")
@app_commands.describe(usuario="Usuario", motivo="Motivo")
@require_key("DIRECTOR_GENERAL", "DIRECTOR_DISCIPLINA", "OWNER", "CO_OWNER")
async def ooc_kick(interaction: discord.Interaction, usuario: discord.Member, motivo: str):
    if not permisos.puede_actuar_sobre(interaction.user, usuario):
        await interaction.response.send_message("❌ No puedes expulsar a alguien de tu nivel o superior.", ephemeral=True)
        return
    try:
        await usuario.kick(reason=f"[OOC] {motivo} — por {interaction.user}")
    except discord.Forbidden:
        await interaction.response.send_message("❌ No tengo permiso de Kick.", ephemeral=True)
        return
    registros.registrar_evento_cargo(usuario.id, "ooc_kick", motivo, interaction.user.id)
    embed = crear_embed(
        "error", "👢 Kick fuera de RP",
...
@bot.tree.command(name="ooc_ban", description="[Fuera de RP] Ban del servidor por infracción grave")
@app_commands.describe(usuario="Usuario", motivo="Motivo", borrar_dias="Días de mensajes a borrar (0-7)")
@require_key("DIRECTOR_GENERAL", "OWNER", "CO_OWNER")
async def ooc_ban(interaction: discord.Interaction, usuario: discord.Member, motivo: str, borrar_dias: int = 0):
    if not permisos.puede_actuar_sobre(interaction.user, usuario):
        await interaction.response.send_message("❌ No puedes banear a alguien de tu nivel o superior.", ephemeral=True)
        return
    borrar_dias = max(0, min(7, borrar_dias))
    try:
        await usuario.ban(reason=f"[OOC] {motivo} — por {interaction.user}", delete_message_days=borrar_dias)
    except discord.Forbidden:
        await interaction.response.send_message("❌ No tengo permiso de Ban.", ephemeral=True)
        return
    registros.registrar_evento_cargo(usuario.id, "ooc_ban", motivo, interaction.user.id)
    embed = crear_embed(
        "error", "🔨 Ban fuera de RP",
        f"**Usuario:** {usuario} ({usuario.id})\n**Motivo:** {motivo}",
        autor=interaction.user,
    )
    await interaction.response.send_message(embed=embed)
...
@bot.tree.command(name="solicitud_degrado", description="Solicita degradado de un miembro (aprobación RRHH)")
@app_commands.describe(usuario="Usuario", cargo_actual="Cargo actual", cargo_propuesto="Cargo propuesto", motivo="Motivo", destinatario="A quién se envía")
@app_commands.choices(destinatario=DESTINATARIO_CHOICES)
@require_key("DIRECTOR", "DIRECTOR_DISCIPLINA", "DIRECTOR_GENERAL", "JEFE_DEPARTAMENTO", "OWNER")
async def solicitud_degrado(interaction: discord.Interaction, usuario: discord.Member, cargo_actual: str, cargo_propuesto: str, motivo: str, destinatario: app_commands.Choice[str]):
    embed = crear_embed(
        "aviso", "⬇️ Solicitud de degradado",
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            ...
            datos={
                ...
                "cargo_propuesto": cargo_propuesto,
                "motivo": motivo,
            },
            canal_key=canal_para_key(destinatario.value),
        )
        await interaction.followup.send(
            f"✅ Solicitud de degradado de {usuario.mention} enviada a **{nombre_destinatario(destinatario.value)}**.",
            ephemeral=True,
        )
    except Exception as e:
        await interaction.followup.send(f"❌ Error: `{e}`", ephemeral=True)
...
@bot.tree.command(name="solicitud_general", description="Envía una solicitud a cualquier dirección del hospital")
@app_commands.describe(
    destinatario="Dirección a la que se envía",
    asunto="Asunto",
    contenido="Contenido de la solicitud",
)
@app_commands.choices(destinatario=DESTINATARIO_CHOICES)
@require_key("STAFF", "SUPERVISOR", "DIRECTOR", "JEFE_DEPARTAMENTO", "OWNER")
async def solicitud_general(
    interaction: discord.Interaction,
    ...
    try:
        await enviar_solicitud_con_aprobacion(
            ...
            datos={"asunto": asunto, "contenido": contenido, "destinatario": key_dest},
            canal_key=canal_para_key(key_dest),
        )
        await interaction.followup.send(
            f"✅ Solicitud enviada a **{nombre_destinatario(key_dest)}**.",
            ephemeral=True,
        )
    except Exception as e:
        await interaction.followup.send(f"❌ Error al enviar: `{e}`", ephemeral=True)
