# -*- coding: utf-8 -*
"""cs_ui_patches — vistas limpias de solicitudes + logs RRHH."""
...
def apply(mod):
    ...
    class TicketSolicitudView(ui.View):
        ...
        @ui.button(label="Aprobar", style=discord.ButtonStyle.success, emoji="✅", custom_id="sol_ok_v6")
        ...
        @ui.button(label="Rechazar", style=discord.ButtonStyle.danger, emoji="❌", custom_id="sol_no_v6")
        async def rechazar(self, inter, btn):
            if not _puede(inter.user):
                return await inter.response.send_message("Solo staff.", ephemeral=True)
            await inter.response.send_modal(MotivoRechazo(self.bot, self.solicitud_id))

        @ui.button(label="Añadir personas", style=discord.ButtonStyle.secondary, emoji="👥", custom_id="sol_add_v6")
        ...
        @ui.button(label="Transcribir y cerrar", style=discord.ButtonStyle.secondary, emoji="📜", custom_id="sol_cls_v6")
    ...
    class PanelDecisionLog(ui.View):
        def __init__(self, bot, sid):
            super().__init__(timeout=None)
            self.bot, self.sid = bot, sid
        @ui.button(label="Aprobar", style=discord.ButtonStyle.success, emoji="✅", custom_id="sol_log_ok_v6")
        async def ap(self, inter, btn):
            v = TicketSolicitudView(self.bot, self.sid)
            await v.aprobar(inter, btn)
        @ui.button(label="Rechazar", style=discord.ButtonStyle.danger, emoji="❌", custom_id="sol_log_no_v6")
        async def re(self, inter, btn):
            if not _puede(inter.user):
                return await inter.response.send_message("Solo staff.", ephemeral=True)
            await inter.response.send_modal(MotivoRechazo(self.bot, self.sid))
        @ui.button(label="Ir al ticket", style=discord.ButtonStyle.secondary, emoji="🎫", custom_id="sol_log_go_v6")
    ...
    print("[cs_ui] OK — Aprobar|Rechazar|Añadir|Cerrar + logs RRHH")
